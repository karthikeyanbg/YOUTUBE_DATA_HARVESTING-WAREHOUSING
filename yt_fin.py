...
def add_bg_from_url():
    st.markdown(
        f"""
         <style>
         .stApp {{
             background-image: url("https://cdn.textstudio.com/output/sample/normal/2/3/7/6/youtube-logo-275-6732.png");
...
with col1:
    ...
    st.write(
        '(Info:- This box **collects data**  using channel id and **stored it in  :green[MongoDB] database**.)')
    channel_id = st.text_input('**Enter  channel_id**')
    st.write('''Get data and stored it in  MongoDB database to click below **:blue['Get data and stored']**.''')
...
with col2:
    ...
    st.write('''(Info:- This zone **Migrate to :blue[MySQL] database from  :green[MongoDB] database** depending on your selection,
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        connect = mysql.connector.connect(
            ...
            auth_plugin="YOUR AUTH_PLUGIN_PASSWORD")
