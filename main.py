...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https://www.shutterstock.com/image-vector/red-play-vector-logo-jpg-jpeg");
...
with col1:
    ...
    st.write(
        '(Note:- This zone **collect data** by using channel id and **stored it in the :blue[MongoDB] database**.)')
    ...
    if "Get_data" or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyDKKqRVuFjMlKtHKbYiC_l1Tz3MGHkn1bs'
        ...
        st.write("Error: Failed to retrieve channel data.")
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='Karthikeyan', db='youtube_data')
